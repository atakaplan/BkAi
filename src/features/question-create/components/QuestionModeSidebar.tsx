...
export function QuestionModeSidebar({ selected, onSelect, onOpenCreatedList }: Props) {
  return (
            ...
            >
              ...
              <span className="min-w-0 flex-1">
                <span className="flex flex-wrap items-center gap-2">
                  <span className={`text-sm font-semibold leading-snug ${isOn ? 'text-slate-900' : 'text-slate-800'}`}>
                    {copy.title}
                  </span>
                  {mode.id === 'ai' ? (
                ...
                <span className="mt-1 block text-xs leading-relaxed text-slate-600">{copy.description}</span>
              </span>
              <Icon
                icon="mdi:chevron-right"
                className={`mt-1 shrink-0 text-2xl transition ${
                  isOn ? 'text-sky-600' : 'text-slate-300 group-hover:text-slate-400'
                }`}
                aria-hidden
              />
            </button>
