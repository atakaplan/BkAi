...
function ReadOnlyDifficultyStep({
  ...
  return (
    <div
      ...
      aria-label={label}
    >
      <span className={`text-center text-xs font-semibold ${labelColor}`}>
        {label}
      </span>
...
export function MetodboxQuestionsStepPanel({
  ...
  const counts = useMemo(
    () => getDifficultyCountsFromChosenIds(chosenIds),
    [chosenIds],
  );

  const question =
    MOCK_METODOBOX_QUESTIONS[activeIndex] ?? MOCK_METODOBOX_QUESTIONS[0];
    ...
    <div className="flex flex-col gap-4">
              ...
              <div className="min-w-0 flex-1 space-y-2">
                <div>
                  ...
                  <p className="text-sm font-semibold text-slate-900">
                    {question.kazanimKodu}
                  </p>
                  <p className="mt-1 text-xs leading-relaxed text-slate-600 sm:text-sm">
                    {question.kazanimMetni}
                  </p>
                </div>
              ...
              <div className="mt-4 flex min-h-0 flex-1 flex-col gap-4">
                ...
                <div className="grid grid-cols-2 gap-2 sm:grid-cols-4">
                  {question.options.map((opt) => (
                    ...
                    >
                      <span className="text-sky-700">{opt.key})</span>{" "}
                      {opt.text}
                    </div>
                  ))}
                </div>
      ...
      {chosenIds.length === 0 ? (
        <p className="text-xs text-slate-500">
          İlerlemek için listeden en az bir soru seçin.
        </p>
      ) : null}
    </div>
  );
