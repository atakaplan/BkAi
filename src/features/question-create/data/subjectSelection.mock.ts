...
export const SUBJECT_SELECTION_OPTIONS: Record<SubjectFieldKey, SubjectOption[]> = {
  level: [
...
export function getSubjectSelectionSummaryLabels(v: Record<SubjectFieldKey, string>): string[] {
  const keys: SubjectFieldKey[] = ['level', 'lesson', 'unit', 'topic']
  return keys
    .map((key) => SUBJECT_SELECTION_OPTIONS[key].find((o) => o.value === v[key])?.label)
