...
function isValidSubjectValue(
  key: SubjectFieldKey,
  value: unknown,
): value is string {
  return (
    typeof value === "string" &&
    SUBJECT_SELECTION_OPTIONS[key].some((o) => o.value === value)
  );
}
...
export function validateQuestionCreateDraft(raw: unknown): QuestionCreateDraft {
  ...
  if (
    ...
    METODOBOX_TEST_CARDS.some((t) => t.id === o.testId)
  ) {
    testId = o.testId;
  }

  const completed: QuestionCreateCompletedSteps = {
    1: false,
    2: false,
    3: false,
  };
  ...
  if (Array.isArray(o.step3ChosenIds)) {
    step3ChosenIds = o.step3ChosenIds.filter(
  ...
  if (
    typeof o.step3ActiveIndex === "number" &&
    Number.isFinite(o.step3ActiveIndex)
  ) {
    step3ActiveIndex = Math.min(
      maxIndex,
      Math.max(0, Math.floor(o.step3ActiveIndex)),
    );
  }
...
export function loadQuestionCreateDraft(
...
export function saveQuestionCreateDraft(
  mode: QuestionCreationModeId,
  ...
  try {
    sessionStorage.setItem(
      storageKey(mode),
      JSON.stringify({ ...draft, v: QUESTION_CREATE_DRAFT_VERSION }),
    );
  } catch {
    /* quota / private mode */
  }
