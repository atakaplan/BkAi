...
export function useQuestionCreateDraft(mode: QuestionCreationModeId) {
  const [draft, setDraft] = useState<QuestionCreateDraft>(() => loadQuestionCreateDraft(mode))
  ...
  const setCompleted = useCallback((updater: (c: QuestionCreateCompletedSteps) => QuestionCreateCompletedSteps) => {
  ...
  const setStep3ChosenIds = useCallback((updater: string[] | ((prev: string[]) => string[])) => {
    setDraft((prev) => ({
      ...prev,
      step3ChosenIds: typeof updater === 'function' ? updater(prev.step3ChosenIds) : updater,
    }))
  }, [])
