...
export function parseMode(raw: string | null): QuestionCreationModeId | null {
  if (raw && MODE_IDS.has(raw as QuestionCreationModeId)) return raw as QuestionCreationModeId
  return null
}

export function parseTab(raw: string | null): QuestionCreateSubTabId | null {
  if (raw && TAB_IDS.has(raw as QuestionCreateSubTabId)) return raw as QuestionCreateSubTabId
  return null
}
